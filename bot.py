...
"
        ...
        "Instagram / Facebook ka public link bhejo."
    )
...
async def download_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_url(url):
        await update.message.reply_text(
            "❌ Sirf Instagram / Facebook ka valid public video link bhejo."
        )
        return

    await update.message.reply_text(
        "⏳ Download ho raha hai, thoda wait karo..."
    )
    ...
    try:
        ...
        # Debug ke liye chaaho to log/print kar sakte ho
        print("Download error:", e)
        await update.message.reply_text(
            "❌ Download failed. Public Insta/Facebook link try karo."
        )
